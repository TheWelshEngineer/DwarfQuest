...
public class CreatureAI {
	...
	protected boolean canUseBetterEquipment() {
		...
		double currentArmorRating = creature.armor() == null ? 0 : creature.armor().defenseValue();
		...
		for(Item item : creature.inventory().getItems()) {
			...
			if(item.damageDice() + item.rangedDamageDice() > currentWeaponRating || isArmor && item.defenseValue() > currentArmorRating) {
				return true;
			}
		}
		return false;
