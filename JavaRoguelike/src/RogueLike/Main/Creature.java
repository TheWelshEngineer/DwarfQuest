...
public class Creature implements Cloneable{
	...
	public void gainDexterity() {
		dexterity += 1;
		doAction("look more agile");
	}

	public void gainIntelligence() {
		intelligence += 1;
		doAction("look more awake");
	}
	...
	public void setTribal(boolean value) {
		tribal = value;
	}
	...
	public void attack(Creature other) {
		...
		if(weapon != null) {
			if(weapon.isFinesse() > 0) {
				attackRoll = this.dexterityRoll()+this.skillFinesseWeapons()+weapon.upgradeLevel();
			}else if(weapon.isMartial() > 0) {
				attackRoll = this.strengthRoll()+this.skillMartialWeapons()+weapon.upgradeLevel();
			}else if(weapon.isSimple() > 0) {
				attackRoll = this.strengthRoll()+this.skillSimpleWeapons()+weapon.upgradeLevel();
			}else {
	...
	private void throwAttack(Item item, Creature other) {
		...
		if(item.isThrownWeapon() > 0) {
			if(item.isFinesse() > 0) {
				attackRoll = this.dexterityRoll()+this.skillFinesseWeapons()+weapon.upgradeLevel();
			}else if(item.isMartial() > 0) {
				attackRoll = this.strengthRoll()+this.skillMartialWeapons()+weapon.upgradeLevel();
			}else if(item.isSimple() > 0) {
				attackRoll = this.strengthRoll()+this.skillSimpleWeapons()+weapon.upgradeLevel();
			}else {
				attackRoll = this.strengthRoll();
			}
	...
	public void rangedWeaponAttack(Creature other) {
		...
		int amount = 0;
		if(weapon != null) {
			amount = (int)(ExtraMaths.diceRoll(weapon.damageDiceLowerBound(), weapon.rangedDamageDice()))+this.dexterityModifier()+weapon.upgradeLevel();
		}else {
			amount = 1+this.dexterityModifier();
		}

		int attackRoll = this.dexterityRoll()+this.skillRangedWeapons()+weapon.upgradeLevel();
		...
		if(item.isWeapon() == 1) {
			if(item == weapon) {
				...
				if(weapon != null && weapon.isCursed() > 0) {
					notify("Your "+nameOf(weapon)+" is cursed! You can't let go of it!");
				}else if((item.skillRestriction() > 0 && item.isMartial() > 0 && (this.skillMartialWeapons() < item.skillRestriction())) || (item.skillRestriction() > 0 && item.isSimple() > 0 && (this.skillSimpleWeapons() < item.skillRestriction())) || (item.skillRestriction() > 0 && item.isFinesse() > 0 && (this.skillFinesseWeapons() < item.skillRestriction())) || (item.skillRestriction() > 0 && item.isRangedWeapon() > 0 && (this.skillRangedWeapons() < item.skillRestriction()))) {
			...
			if(item == armor) {
				...
				if(armor != null && armor.isCursed() > 0) {
					notify("Your "+nameOf(armor)+" is cursed! You can't take it off!");
				}else if(item.skillRestriction() > 0 && item.isFortitude() > 0 && (this.skillFortitude() < item.skillRestriction())) {
					notify("You aren't skilled enough to use the "+nameOf(item)+".");
				}else {
					if(item.isCursed() > 0) {
						if(this.rollPerception() > 15) {
	...
	private void regenerateHealth() {
		...
		if(regenHPCooldown <= 0) {
			...
			if(this.playerClass() == "Warrior") {
				regen = (int)Math.ceil(this.maxHP()*0.15);
			}else if(this.playerClass() == "Rogue") {
				regen = (int)Math.ceil(this.maxHP()*0.10);
			}else if(this.playerClass() == "Mage") {
				regen = (int)Math.ceil(this.maxHP()*0.05);
			}else {
				regen = (int)Math.ceil(this.maxHP()*0.10);
			}
	...
	public void castSpell(Spell spell, int x2, int y2, Item item) {
		...
		if(spell.castOnTile()) {
			...
			if(item != null) {
				if((item.skillRestriction() > 0 && item.isEvocation() > 0 && (this.skillEvocation() < item.skillRestriction())) || (item.skillRestriction() > 0 && item.isPyromancy() > 0 && (this.skillPyromancy() < item.skillRestriction())) || (item.skillRestriction() > 0 && item.isCryomancy() > 0 && (this.skillCryomancy() < item.skillRestriction())) || (item.skillRestriction() > 0 && item.isElectromancy() > 0 && (this.skillElectromancy() < item.skillRestriction())) || (item.skillRestriction() > 0 && item.isAlchemancy() > 0 && (this.skillAlchemancy() < item.skillRestriction()))) {
		...
		if(this.spellbook().isSpellKnown(spell)) {
			...
			if(item.skillRestriction() > 0) {
				if(item.isPyromancy() > 0) {
					if(this.skillPyromancy() >= item.skillRestriction()) {
				...
				}else if(item.isCryomancy() > 0) {
					if(this.skillCryomancy() >= item.skillRestriction()) {
						notify("You learn how to cast "+spell.name()+"!");
						this.spellbook().add(spell);
						if(this.nameOf(item) != item.name()) {
							this.learnName(item);
				...
				}else if(item.isElectromancy() > 0) {
					if(this.skillElectromancy() >= item.skillRestriction()) {
						notify("You learn how to cast "+spell.name()+"!");
						this.spellbook().add(spell);
						if(this.nameOf(item) != item.name()) {
							this.learnName(item);
				...
				}else if(item.isAlchemancy() > 0) {
					if(this.skillAlchemancy() >= item.skillRestriction()) {
						notify("You learn how to cast "+spell.name()+"!");
						this.spellbook().add(spell);
						if(this.nameOf(item) != item.name()) {
							this.learnName(item);
				...
				}else if(item.isEvocation() > 0) {
					if(this.skillEvocation() >= item.skillRestriction()) {
						notify("You learn how to cast "+spell.name()+"!");
						this.spellbook().add(spell);
						if(this.nameOf(item) != item.name()) {
							this.learnName(item);
